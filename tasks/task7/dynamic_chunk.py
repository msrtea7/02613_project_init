...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        ...
        print(f"Process floors from {start_idx} to {end_idx}. Currently {mp.cpu_count()} processes available, using {N_process}")

    building_ids = building_ids[start_idx:end_idx+1]
    N = len(building_ids)
    ...
    with Pool(processes=N_process) as pool:
        for i, u in pool.map(cal_temp, args_list):
            all_u[i] = u
